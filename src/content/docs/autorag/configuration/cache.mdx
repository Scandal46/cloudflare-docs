...
This method prioritizes phrasing and word overlap—not semantic meaning. For example, `"What’s the weather today?"` and `"How’s the weather today?"` may match, while `"Is it hot outside?"` may not, even if the intent is similar.
