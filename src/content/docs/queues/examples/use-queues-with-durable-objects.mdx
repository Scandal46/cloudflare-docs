...
The `constructor()` in the Durable Object makes your `Environment` available (in scope) on `this.env` to the [`fetch()` handler](/durable-objects/features/create-durable-object-stubs-and-send-requests/) in the Durable Object.
